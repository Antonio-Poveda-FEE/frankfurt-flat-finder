...
import { buildRecommendation, computeFlatScores } from '../lib/stats'
...
export default function Compare() {
  ...
  const ranked = useMemo(() =>
    flats
      .map((f) => ({ flat: f, score: scoreMap.get(f.id)?.global ?? null }))
  ...
  const radarData = useMemo(() => {
    return criteria.map((c) => {
      ...
      for (const id of effectiveSelected) {
        ...
        if (f) row[f.title] = scoreMap.get(id)?.perCriterion100[c.id] ?? 0
      }
      return row
    })
  }, [criteria, effectiveSelected, flats, scoreMap])
      ...
      {ranked.length === 0 ? (
        ...
        <>
          ...
          <section className="overflow-hidden rounded-2xl bg-slate-900 ring-1 ring-slate-800">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {ranked.map((r, i) => {
                  ...
                  const ppm2 = pricePerM2(costs[r.flat.id], r.flat.size_m2)
                  ...
                  return (
                    <tr key={r.flat.id} className="border-t border-slate-800/60">
                      ...
                      <td className="px-2 py-2 text-right text-slate-400">{ppm2 != null ? num(ppm2) : '—'}</td>
                      <td className="px-3 py-2 text-right font-bold" style={{ color: scoreColor(r.score) }}>{r.score!.toFixed(0)}</td>
            ...
            <p className="px-4 py-2 text-[11px] text-slate-500">Marca hasta 5 pisos para superponerlos en el radar.</p>
          </section>

          {/* Radar */}
          {effectiveSelected.length > 0 && (
            <section className="rounded-2xl bg-slate-900 p-4 ring-1 ring-slate-800">
              <h2 className="mb-2 font-semibold text-white">Comparativa por criterio</h2>
              <div className="h-96">
